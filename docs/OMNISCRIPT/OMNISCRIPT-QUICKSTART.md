# OmniScript Quickstart — NovaShield
## The Accountability Dimension
### © 2024–2026 Jeffrey W Williams LLC. All Rights Reserved.

---

> This guide gets you writing OmniScript for **NovaShield** in under 10 minutes.
...
# Initialize OmniScript in NovaShield
cd novashield
omni init
...
```omni
// NovaShield/omniscript/hello.omni
// © 2024–2026 Jeffrey W Williams LLC. All Rights Reserved.

draw { Nova } from "nova:std"
draw { Vibe, Emotion } from "nova:std/types"

/// Greet a Dimensional Citizen entering The Accountability Dimension
manifest greetCitizen(userId: Text): flow<Text> {
  ...
  forge message = when (vibe.intensity > 80.0%) {
    "Welcome, Dimensional Citizen. Your energy is high — The Accountability Dimension responds to you."
  } otherwise when (emotion.primary == EmotionClass.PEACE) {
    "Welcome. The The Accountability Dimension is calm and ready for your presence."
  } otherwise {
    "Welcome to NovaShield. The The Accountability Dimension activates around you."
  }

  Nova.Log.info(`Citizen ${userId} entered The Accountability Dimension: ${message}`)
  propagate message
}
```
...
```omni
// NovaShield/omniscript/my-first-engine.omni
// © 2024–2026 Jeffrey W Williams LLC. All Rights Reserved.

draw { Nova } from "nova:std"
draw { Engine, EngineResult } from "nova:std/engine"

@engine(id: "my-first-engine", platform: "NovaShield", version: "1.0.0")
engine MyNovaShieldEngine implements Engine {
...
```omni
draw { MyNovaShieldEngine } from "./my-first-engine"
Nova.Engine.register(MyNovaShieldEngine)
```

---

## Platform-Specific Example: ConductSignalEngine

```omni
// NovaShield/omniscript/engines/ConductSignalEngine.omni
// © 2024–2026 Jeffrey W Williams LLC. All Rights Reserved.

draw { Nova } from "nova:std"
draw { Engine, EngineResult } from "nova:std/engine"

@engine(id: "conductsignalengine-v1", platform: "NovaShield", version: "1.0.0", priority: Priority.CRITICAL)
engine ConductSignalEngine implements Engine {
  ...
  manifest report(): EngineReport { propagate EngineReport { engineId: "ConductSignalEngine", status: Status.ACTIVE } }
}
```
...
Nova.Bus.emit("shield.incident.high-severity", Signal {
  ...
  payload: { userId: "nova-user-alpha", platform: "NovaShield" },
  origin:  "novashield",
...
2. Explore [TERMINOLOGY-GLOSSARY.md](./TERMINOLOGY-GLOSSARY.md) for NovaShield terminology
